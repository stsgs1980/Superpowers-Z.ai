...
- User has given specific instructions: `{argument}`
...
```
{
  ...
  "prompt": "You are analyzing a conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing the assistant's actions)
...
After gathering behaviors (from arguments or agent), present to user:
...
Use the current working directory (where the assistant was started) as the base path.
